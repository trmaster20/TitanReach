...
static int THE_STRANGER = 92;
static int BUCK = 94;
static int FRITZ = 95;
static int CHARSI = 93;
static int JACK = 98;
static int MURRAY = 99;
static int NIKA = 100;
static int JEN = 110;
...
Func<Npc, Player, int, CancellationToken, Task<int>> StrangerDialog = async (npc, player, s, token) => {
    ...
    if (QS > 2 && QS < 50) {
        switch (s) {
            ...
            case (1):
                await Task.Run(() => Talk(npc, player, token, "Talk to " + NpcLink(JACK) + ", he's located south across the river at the lumbermill. He'll get you sorted out."), token);
    ...
    switch (QS) {
        case 2:
            ...
            switch (choice) {
                case 0:
                    ...
                    await Task.Run(() => Talk(npc, player, token, "Great, go and talk to " + NpcLink(JACK) + " across the river to the south. he'll show you how how to gather the logs."), token);
                    await Task.Run(() => Talk(player, npc, token, "Ok, thanks."), token);
                    return EXIT;
                case 1:
        ...
        case 90:
            ...
            await Task.Run(() => Talk(npc, player, token, "Awesome work, you can use fires to cook food. On that note, go and speak to " + NpcLink(FRITZ) + ". You can find him on the other side of the river if you follow the path west. He might be able to help you find some food."), token);
        ...
        case 100:
            ...
            await Task.Run(() => Talk(npc, player, token, "Go and speak to " + NpcLink(FRITZ) + ". You can find him on the other side of the river if you follow the path west. He might be able to help you find some food."), token);
            await Task.Run(() => Talk(player, npc, token, "Thanks for your help!"), token);
            return EXIT;
    ...
    switch (QS) {
        ...
        case 120:
            if (HasInventory(player, DataManager.ItemID.CHUB, 1)) {
                ...
                await Task.Run(() => Talk(npc, player, token, "Awesome work! I've taught you what I can for now. Why don't you go and see " + NpcLink(BUCK) + " next? You'll find him in the copper mine to the North East, up past the strangers house, and down the path." +
        ...
        case 130:
            await Task.Run(() => Talk(player, npc, token, "I've forgotten what I was supposed to be doing"), token);
            await Task.Run(() => Talk(npc, player, token, "Why don't you go and see " + NpcLink(BUCK) + " next? You'll find him in the copper mine to the North East, up past the strangers house, and down the path" +
                    " He might be able to show you how to get a weapon to help defend yourself."), token);
            await Task.Run(() => Talk(player, npc, token, "Ok, see you later."), token);
                return EXIT;
    }
    ...
    switch (QS) {
        ...
        case 140:
            if (HasInventory(player, DataManager.ItemID.COPPER_ORE, 8)) {
                ...
                await Task.Run(() => Talk(npc, player, token, "There is a forge back up the path to the west, head over there and talk to " + NpcLink(CHARSI) + ". They should be able to help you out."), token);
                return EXIT;
    ...
    switch (QS) {
        ...
        case 152:
            if (HasInventory(player, DataManager.ItemID.COPPER_DAGGER, 1)) {
                ...
                await Task.Run(() => Talk(npc, player, token, "Perfect, you'll be making Titanite armour in no time. For now though, you'll need more than just that " + ItemLink(DataManager.ItemID.COPPER_DAGGER) + " to survive the dangers of the world.| Go and see " + NpcLink(MURRAY) + " just to the west."), token);
                await Task.Run(() => Talk(player, npc, token, "Great, thanks for your help."), token);
                return EXIT;
        ...
        case 160:
            await Task.Run(() => Talk(player, npc, token, "I've forgotten what I was supposed to be doing"), token);
            await Task.Run(() => Talk(npc, player, token, "Go and see " + NpcLink(MURRAY) + " just to the west of here"), token);
            await Task.Run(() => Talk(player, npc, token, "Great, thanks for your help."), token);
            return EXIT;
    }
    ...
    switch (QS) {
        ...
        case 180:
            if (HasInventory(player, DataManager.ItemID.SHORTBOW_UNSTRUNG, 1)) {
                ...
                await Task.Run(() => Talk(npc, player, token, "A bow is made from an unstrung bow and a bow string. Firstly go and get " + ItemLink(DataManager.ItemID.LOGS) + ", then craft them into an " + ItemLink(DataManager.ItemID.SHORTBOW_UNSTRUNG) + " at a workbench just like this one next to me."), token);
                await Task.Run(() => Talk(player, npc, token, "Ok, be right back"), token);
                return EXIT;
            }
        ...
        case 190:
            if (HasInventory(player, DataManager.ItemID.BOW_STRING, 1)) {
                ...
                await Task.Run(() => Talk(npc, player, token, "Go and pick some " + ItemLink(DataManager.ItemID.FLAX) + " which grows just down the hill to the west, then turn it into a " + ItemLink(DataManager.ItemID.BOW_STRING) + " at a spinning wheel. You can find one near the workbench."), token);
                await Task.Run(() => Talk(player, npc, token, "Ok, be right back."), token);
                return EXIT;
            }
        ...
        case 230:
            if (HasInventory(player, DataManager.ItemID.COPPER_ARROW, 30)) {
                ...
                await Task.Run(() => Talk(npc, player, token, "Awesome! You look like you're sorted for weapons for the time being. " + NpcLink(NIKA) + " should be able to help you make some armour. She is located in the north east corner of Glimmermoore town"), token);
        ...
        case 240:
            await Task.Run(() => Talk(player, npc, token, "I've forgotten what I was supposed to be doing"), token);
            await Task.Run(() => Talk(npc, player, token, "Go and see " + NpcLink(NIKA) + ". She is located in the nortth east corner of Glimmermoore town, they'll help you find something a bit more protective to wear."), token);
            await Task.Run(() => Talk(player, npc, token, "Great, thanks for your help."), token);
            return EXIT;
    }
    return EXIT;
    ...
    switch (QS) {
        ...
        case 250:
            if (HasInventory(player, DataManager.ItemID.COWHIDE, 1)) {
                ...
                await Task.Run(() => Talk(npc, player, token, "You'll need animal hide to turn into leather, you can get " + ItemLink(DataManager.ItemID.COWHIDE) + " from killing cows, i saw a few just to the east"), token);
                await Task.Run(() => Talk(player, npc, token, "Ok, be right back."), token);
                return EXIT;
            }
        ...
        case 280:
            if (HasInventory(player, DataManager.ItemID.LEATHER_GLOVES, 1)) {
                ...
                await Task.Run(() => Talk(npc, player, token, "Awesome! You've got a fair amount of stuff now, maybe " + NpcLink(JEN) + " at the bank can help you store your items. The bank is directly south."), token);
